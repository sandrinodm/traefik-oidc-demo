...
1. **Traefik Proxy** runs two local Go middleware plugins in its own process: [`sevensolutions/traefik-oidc-auth`](https://github.com/sevensolutions/traefik-oidc-auth) enforces OIDC, while the repository's `oidc-provider-selector` plugin renders the provider page, starts provider-specific login attempts, and handles local logout.
...
```text
No authenticated provider session
Browser ──> Traefik selector (/login) ──> /login/provider-1 or /login/provider-2
                                                │
                                                └── matching OIDC middleware creates encrypted state + PKCE

Provider callback
/oauth2/callback/provider-N ──> matching OIDC middleware ──> encrypted provider-N session

Later requests
Provider-N session cookie ──> matching OIDC middleware ──> Express header viewer
```

1. Without a provider session, a low-priority Traefik router redirects the browser to `/login?return_to=…`. That public URL always renders the selector, so a user can return to it to switch providers.
2. With multiple enabled providers, the selector posts to a provider-specific login route. With one provider, it redirects there without showing the choice page. No provider-selection cookie is created.
3. The matching OIDC middleware creates and validates the encrypted `state` value and PKCE verifier for that specific login attempt. Callback paths are isolated by provider.
4. A higher-priority `HeaderRegexp` router invokes exactly one OIDC middleware when it finds that provider's encrypted session cookie. The plugin still performs discovery, signature, expiry, issuer, and audience validation.
...
| `OIDC_COOKIE_SECURE` | no | `true` | Traefik | Mark OIDC cookies HTTPS-only |
...
- `/logout` is handled by the selector middleware. It clears visible chunks of both local OIDC sessions (and the legacy selector cookie, if present), but does not end the provider's SSO session.
